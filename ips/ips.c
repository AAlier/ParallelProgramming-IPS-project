...
void ips_apply_sobel(ips_task_t *task);
...
/* Threading Data */
static pthread_mutex_t Resource_Counter_Mutex = PTHREAD_MUTEX_INITIALIZER;		
static pthread_cond_t Got_Resources_Condition = PTHREAD_COND_INITIALIZER;
...
void ips_apply_sobel(ips_task_t *task){
    ...
    for (y = task->row_index_to_process; y < task->last_row_index_to_process; ++y) {
        for (x = 0; x < output_image->width; ++x) {
            ...
            if( output_image->height != NULL && output_image->width != NULL && 
				input_image->height != NULL && input_image->width != NULL && 
				y != 0 && y != output_image->height - 1 && x != 0 && x != output_image->width - 1) {
                for (channel = 0; channel < 3; ++channel) {
                    gx +=    ((png_bytep)&(input_image->rows[y - 1][(x - 1) * channels]))[channel] + 
                        2 * ((png_bytep)&(input_image->rows[y][(x - 1) * channels]))[channel] + 
                            ((png_bytep)&(input_image->rows[y + 1][(x - 1) * channels]))[channel] -
                            ((png_bytep)&(input_image->rows[y - 1][(x + 1) * channels]))[channel] - 
                        2 * ((png_bytep)&(input_image->rows[y - 1][(x - 1) * channels]))[channel] -
                            ((png_bytep)&(input_image->rows[y + 1][(x + 1) * channels]))[channel];
                    gy +=    ((png_bytep)&(input_image->rows[y - 1][(x - 1) * channels]))[channel] + 
                        2 * ((png_bytep)&(input_image->rows[y - 1][(x) * channels]))[channel] + 
                            ((png_bytep)&(input_image->rows[y - 1][(x + 1) * channels]))[channel] -
                            ((png_bytep)&(input_image->rows[y + 1][(x - 1) * channels]))[channel] - 
                        2 * ((png_bytep)&(input_image->rows[y + 1][(x) * channels]))[channel] -
                            ((png_bytep)&(input_image->rows[y + 1][(x + 1) * channels]))[channel];
...
void *ips_thread_process_image_part(void *args)
{
    ...
    ips_task_t *task;
	pthread_mutex_lock(&Resource_Counter_Mutex);
	pthread_cond_wait(&Got_Resources_Condition, &Resource_Counter_Mutex);
	pthread_mutex_unlock(&Resource_Counter_Mutex);
    ...
    while (pool->size != 0) {
        ...
        if (pool->size > 0) {
            // Get a new task
			//printf("Consumer %d: locking the mutex\n", pool->size);
			task = pool->first_task;
            pool->first_task = task->next_task;
            pool->size--;
            //printf("Consumer: decrementing pools size to %d\n", pool->size);
			if (!pool->size) {
                pool->last_task = NULL;
            }
...
void ips_update_matrices(int window_width, int window_height)
{
    view_matrix =
        glm::lookAt<float>(
